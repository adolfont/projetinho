defmodule ProjetinhoWeb.PageLive do
  ...
  def handle_event("keydown", %{"key" => "ArrowLeft"}, socket) do
    {:noreply, update(socket, :x, &left_or_up/1)}
  end

  defp left_or_up(x) when x == 0, do: x
  defp left_or_up(x), do: x - 1

  def handle_event("keydown", %{"key" => "ArrowRight"}, socket) do
    {:noreply, update(socket, :x, &right/1)}
  end

  defp right(x) when x == 25, do: x
  defp right(x), do: x + 1

  def handle_event("keydown", %{"key" => "ArrowDown"}, socket) do
    {:noreply, update(socket, :y, &down/1)}
  end

  defp down(x) when x == 13, do: x
  defp down(x), do: x + 1

  def handle_event("keydown", %{"key" => "ArrowUp"}, socket) do
    {:noreply, update(socket, :y, &left_or_up/1)}
  end
